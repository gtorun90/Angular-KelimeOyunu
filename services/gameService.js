...
router.get("/index", user.checkAuthenticated, async (req, res) => {
  var questionBank = await QuestionBank.find({}, "-__v").skip();
  var filteredQuestionBank = [];
  for (var i = 4; i < 11; i++) {
    var topLimit =
      questionBank.filter((x) => x.cevapharfsayisi == i).length - 1;
    var firstIndex = Math.abs(Math.floor(Math.random() * topLimit));
    var randomArray = questionBank
      .filter((x) => x.cevapharfsayisi == i && x.soruldu == false)
      .sort(function (a, b) {
        return 0.5 - Math.random();
      })
      .slice(firstIndex, firstIndex + 2); //aynı harf sayılı sorulardan rastgele 2 tanesini alma
    randomArray.map((x) => {
      // Alınan 2 li sorular yeni bir diziye aktarılıp return ediliyor
  ...
  filteredQuestionBank.sort((a, b) =>
    a.cevapHarfSayisi > b.cevapHarfSayisi
      ? 1
      : a.cevapHarfSayisi === b.cevapHarfSayisi
      ? 1
      : -1
